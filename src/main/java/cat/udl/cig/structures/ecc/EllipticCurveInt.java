...
public interface EllipticCurveInt extends Group {
     ...
     * \(E\).
     *
     * @param P the <i>ECPoint</i> to be checked.
     ...
     * Curve</i> \(E\).
     *
     * @param x a RingElement representing the first coordinate of the point
     *          \(P\).
     * @param y a RingElement representing the second coordinate of the point
     *          \(P\).
     ...
     *
     * @param x the RingElement we want to check if it belongs to a point \(P
     *          \in E(K)\).
    ...
    default boolean validHasseTheorem(BigInteger probableCardinality) {
        BigInteger fieldSize = getRing().getSize();
        BigInteger leftSide = probableCardinality.subtract(fieldSize.add(BigInteger.ONE)).abs();
